...
Sys.setlocale("LC_TIME", "C")  ### set the time format

### transform to sparse matrix
setM = function(D, vid, score) {
...
### function for get the network structure: Adjency mat
getNetStruc <- function(file) {
...
### obtain regression dataset
getRegDf <- function(logYmatNchar, W, Nodal) {
    Time = ncol(logYmatNchar)
    tt = 1:Time
    logYmatNchar1 = W %*% logYmatNchar
    Zmat = do.call(rbind, rep(list(Nodal), Time - 1))
    
    ### obtain all variables in X
    X = cbind(network = as.vector(logYmatNchar1[, tt[-length(tt)]]), momentum = as.vector(logYmatNchar[, 
        tt[-length(tt)]]), Zmat)
